...
class stream(author.GetAcess):
    ...
    '''
    def stream_topic(self, s_time, e_time, *topic_id, **kwargs):
        # REST API, request data
        with open('request/stream.json', encoding='UTF-8') as f:
        ...
        for row in res.json()['rows']:
            res_dict['Mention'].append(row[0]['actualText'])
            res_dict['Sentiment'].append(row[0]['workflowProperties']['sentiment'])
            


        # DataFrame
        df_Mention = pd.DataFrame(res_dict)
